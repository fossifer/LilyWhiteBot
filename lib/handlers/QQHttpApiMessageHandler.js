...
class QQHttpApiMessageHandler extends MessageHandler {
    ...
    getNick(user, withTitle) {
        if (user) {
            let { user_id, nickname, card, title } = user.sender;
            user_id = user_id || '';
            if (!withTitle || !this._showTitle || !title) { title = '' }
            else { title = `<${title}> ` }

            if (this._nickStyle === 'groupcard') {
                return title + (card || nickname || user_id.toString());
            } else if (this._nickStyle === 'nick') {
                return title + (nickname || user_id.toString());
            } else {
