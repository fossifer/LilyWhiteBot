...
class QQHttpApiMessageHandler extends MessageHandler {
        ...
        client.on('notice', async (context) => {
            switch (context.notice_type) {
                ...
                case 'group_decrease':
                    ...
                    this.emit('leave', {
                        group: parseInt(context.group_id),
                        adminQQ: parseInt(context.operator_id), // 管理員 QQ，自行離開時為 0
                        target: parseInt(context.user_id),
    ...
    getNick(user) {
        if (user) {
            let { user_id, nickname, card } = user.sender;
            user_id = user_id || '';

            if (this._nickStyle === 'groupcard') {
                return card || nickname || user_id.toString();
            } else if (this._nickStyle === 'nick') {
                return nickname || user_id.toString();
            } else {
