...
class QQOICQMessageHandler extends MessageHandler {
    constructor(config = {}) {
        ...
        client.on('message', async (rawdata) => {
            ...
            if (parsedMsg.extra.reply) {
              ...
              let msg = this._msgs.get(id);
              // TODO: 檢查reply id是否為被回覆消息的id
              // 目前來看似乎不是?
              if (msg) {
                let reply = {
                    nick: msg.nick,
            ...
            this._msgs.set(rawdata.message_id, context);
    ...
    _convertToText(message) {
        if (message.text) {
            return message.text;
        } else if (message.extra.files.length) {
          if (message.extra.files[0].type==="photo") {
            return '<Photo>';
          } else if (message.extra.files[0].type==="record") {
            return '<Record>'
          }
        } else {
            return '<Message>';
