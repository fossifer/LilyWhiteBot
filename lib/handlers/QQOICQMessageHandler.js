...
class QQOICQMessageHandler extends MessageHandler {
    constructor(config = {}) {
        ...
        // 配置文件兼容性
        for (let key of ['qq', 'passwordMd5', 'platform', 'logLevel', 'kickoff', 'ignoreSelf']) {
        ...
        this._ignoreSelf = botConfig.ignoreSelf || true;
        ...
        client.on('system.login.captcha', (info)=>{
        ...
        client.on('system.login.device', (info)=>{
          winston.info(`QQBot device lock unlocking required: ${info.url}`);
        })

        client.on('system.login.error', (info)=>{
          winston.info(`QQBot Error: ${info.message}`);
        })
    ...
    getTitle(user) {
        if (user) {
            ...
            if (!title && !honor) { title = '' }
            else { title = title||honor }
            return title;
        } else return '';
    }
    ...
    async groupMemberInfo(group, qq) {
        const info = await this._client.getGroupMemberInfo(group, qq, true);
        return {
            group: info.group_id,
            qq: info.user_id,
            name: info.nickname,
    ...
    async strangerInfo(qq) {
        const info = await this._client.getStrangerInfo(qq);
        return {
            qq: info.user_id,
            name: info.nickname,
