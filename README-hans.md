...
4. 再次启动酷 Q，登入机器人帐号，然后在插件设定中启用「cqsocket」。
...
3. 酷 Q 模拟的是安卓 QQ，而且 QQ 不允许多个手机同时登入。如果已经开启酷 Q，而且需要直接操作机器人帐号，请用电脑登入。
4. 酷 Q 是私有软体，和我没关系。
5. 酷 Q 可以透过 wine 在 Linux/Mac 系统中执行，可以参考[这篇教程](https://cqp.cc/t/30970)进行设定。

### 设定 Telegram 机器人
@BotFather，与其交互，按照荧幕提示进行操作，建立一个机器人帐号。设定完成后，BotFather 会给一个 Token，你需要把这个 Token 填到 config.js 中。
